...
- 按价格从低到高重新整理鱼类、鱼饵和标准商店物品排序，修复 `/商店 1` 展示顺序与价格顺序不一致的问题。
- 重构标准商店：默认上架 3 星及以下鱼竿和鱼饵，并同步基础商店、卡池、商店成本和奖励数据到初始化配置。
- WebUI 默认开启并支持插件启动时自动启动，同时补充共享数据库路径配置，便于多实例共用同一套钓鱼数据。
- 新增/完善鱼饵出售能力和帮助文案，背包、帮助列表与命令行为保持一致。
